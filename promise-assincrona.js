...
/* olhando na aba network do browser, e clicando na requisição random, veremos na aba preview, que ela retorna um objeto com as propriedade message e status, portanto o método fetch retorna uma promise */

/* olhando no console, veremos que a Promise está com o status pendente(pending), que indica que a operação dessa promisse não foi concluída com sucesso nem falhou, e é somente seu estado inicial - e olhando dentro do prototype vemos os métodos then e catch */
...
fetch(url)
    .then(dogData => {
        ...
        /* console.log(dogData) - aqui podemos observar no console que a resposta da requisição foi um objeto response. Esse tipo de objeto é obtido como retorno de uma operação que envolve uma API.
        Agora para obtermos as propriedades do Objeto (message e status), precisamos "Parsear"³ o texto do Body desta resposta como JSON, e para fazer isso, dentro do then podemos declarar um return que retornará esse objeto dogData.json()  */
        return dogData.json() /* e como o retorno de um then é uma nova Promisse, podemos encadear outro then, recebendo esse return como parâmetro, e assim sequencialmente se necessário */
    })
    ...
    .catch(error => {
        alert(error.message)
    }) /* lançando o método catch, que recebe uma função () => {}, que recebe como parâmetro o error (lançado no if do then). Dentro da função especificamos a menssagem do erro.
